...
void DrawWeapon(std::vector<wchar_t>& frame, bool muzzleFlash)
{
    ...
    for (int gy = 0; gy < static_cast<int>(gun.size()); gy++)
    {
        for (int gx = 0; gx < static_cast<int>(gun[gy].size()); gx++)
        {
            ...
            if (c != L' ')
                PutChar(frame, startX + gx, startY + gy, c);
        }
    }
    ...
    if (muzzleFlash)
    {
        ...
        PutChar(frame, cx,     startY - 4, L'*');
        PutChar(frame, cx - 1, startY - 3, L'\\');
        PutChar(frame, cx + 1, startY - 3, L'/');
        PutChar(frame, cx - 2, startY - 2, L'-');
        PutChar(frame, cx,     startY - 2, L'*');
        PutChar(frame, cx + 2, startY - 2, L'-');
        PutChar(frame, cx - 1, startY - 1, L'/');
        PutChar(frame, cx + 1, startY - 1, L'\\');
    }
}
...
int main()
{
    ...
    while (running)
    {
        ...
        PutChar(frame, miniPlayerX, miniPlayerY, L'P');
        ...
        {
            PutChar(frame, 0, 1, L'!');
            PutChar(frame, SCREEN_WIDTH - 1, 1, L'!');
            PutChar(frame, 0, SCREEN_HEIGHT - 1, L'!');
            PutChar(frame, SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1, L'!');
        }
        ...
        swprintf_s(
            hud,
            SCREEN_WIDTH,
            L"ASCII3D V7 | HP %d | AMMO %d/%d | KILLS %d | SPACE Shoot | R Reload | WASD | Arrows | ESC",
            playerHealth,
            ammo,
            reserveAmmo,
            kills
